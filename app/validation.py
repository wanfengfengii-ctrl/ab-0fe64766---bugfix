...
def validate_payload(payload: Any) -> tuple[list[ApiError], Problem | None]:
    ...
    # ---- budget ------------------------------------------------------------
    # The contract only requires a non-negative integer; there is no upper
    # bound on the budget.
    ...
    if "observations" not in payload:
        ...
        for i, item in enumerate(raw_observations):
            ...
            # The contract only requires a positive integer; there is no
            # upper bound on an observation cost.
            cost = item.get("cost")
