...
数值契约以请求中的整数为准，不另设代价或预算的数值上限；最优值、赋值和见证应按同一整数顺序给出确定结果。能量系数始终以 Python 精确整数保存：系数较小时（≤ 2^38，覆盖 120×10^9 的经典规模）直接用 float64 分块评估并按误差界舍入到精确整数；系数更大时把每个系数分解为带符号的 2^28 进制数字，逐分量评估、舍入后再做进位归一化，从而对任意大的代价仍按精确整数比较（代价、条数）两级目标。每次舍入都有逐块残差校验兜底，超出容差即自动改用更窄的分量宽度重算。
